"""Static checks for the V1-G58 built-in provider SDK client authority request."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g58_built_in_provider_sdk_client_authority_contract_approval_request.json"
)
...
def test_v1_g58_request_docs_and_fixture_exist() -> None:
    fixture = _load_fixture()

    assert fixture["gate_id"] == (
        "v1_g58_built_in_provider_sdk_client_authority_contract_approval_request"
    )
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == (
        "prepare-v1-g58-built-in-provider-sdk-client-authority-contract-approval-request"
    )
...
def test_v1_g58_has_no_implementation_approval_yet() -> None:
    fixture = _load_fixture()
    decision = fixture["decision_record"]

    assert fixture["implementation_approved"] is False
    assert fixture["operator_approval_recorded"] is False
    assert fixture["built_in_provider_sdk_client_authority_contract_approved"] is False
    assert fixture["built_in_provider_sdk_client_authority_contract_added"] is False
...
def test_v1_g58_exact_decision_options_are_locked() -> None:
    fixture = _load_fixture()
...
def test_v1_g58_target_if_approved_is_metadata_only_and_deny_by_default() -> None:
    target = _load_fixture()[
        "built_in_provider_sdk_client_authority_contract_target_if_operator_says_yes"
    ]

    assert target["metadata_only_authority_contract_allowed"] is True
    ...
    assert target["provider_capability_declaration_metadata_required"] is True
    ...
    assert (
        target["links_v1_g48_v1_g53_v1_g54_v1_g55_v1_g56_v1_g57_evidence_required"]
        is True
    )
...
def test_v1_g58_approved_file_scope_if_approved_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g58_prior_evidence_refs_exist() -> None:
    for relative_path in _load_fixture()["required_prior_evidence_refs"]:
        assert (REPO_ROOT / relative_path).exists(), relative_path


def test_v1_g58_forbidden_boundaries_remain_false_in_request() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["provider_capability_declaration_metadata_only"] is True
    assert fixture["sdk_dependency_declaration_metadata_only"] is True


def test_v1_g58_docs_contain_request_only_boundary_language() -> None:
    fixture = _load_fixture()
    ...
    assert "Built-in provider SDK client implementation approved: no" in approval_text
    ...
    assert "Implementation must not start until `Approve-V1-G58`" in preflight_text
    assert "No `lima/` runtime files may be changed." in work_order_text


def test_v1_g58_docs_and_fixture_do_not_include_sensitive_markers() -> None:
    fixture = _load_fixture()
