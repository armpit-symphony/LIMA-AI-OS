"""Static checks for the V1 runtime readiness rollup through G58."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_runtime_readiness_rollup_through_g58.json"
)
...
def test_runtime_readiness_rollup_g58_fixture_and_docs_exist() -> None:
    fixture = _load_fixture()

    assert fixture["rollup_id"] == "v1_runtime_readiness_rollup_through_g58"
...
def test_runtime_readiness_rollup_g58_required_verdicts() -> None:
...
def test_runtime_readiness_rollup_g58_current_status_advances_with_g58_evidence() -> None:
    current = _load_fixture()["current_status"]

    assert current["latest_completed_gate"] == "V1-G58"
    assert current["latest_independent_audit"] == "V1-G58"
    assert current["latest_readiness_rollup"] == "V1-G58"
    assert current["current_gate"] == (
        "sdk_dependency_and_vendor_provider_sdk_import_authority_request"
    )
...
def test_runtime_readiness_rollup_g58_status() -> None:
    status = _load_fixture()["g58_status"]

    assert status["built_in_provider_sdk_client_authority_contract_approved"] is True
    assert status["built_in_provider_sdk_client_authority_contract_implemented"] is True
    assert status["built_in_provider_sdk_client_authority_contract_audited"] is True
...
def test_runtime_readiness_rollup_g58_blocked_authorities_remain_false() -> None:
    for key, value in _load_fixture()["blocked_authorities"].items():
        assert value is False, key


def test_runtime_readiness_rollup_g58_validation_evidence_is_recorded() -> None:
    validation = _load_fixture()["validation_evidence"]

    assert validation["focused_v1_g58_validation"] == {
        "passed": True,
        "tests_passed": 10,
    }
    assert validation["focused_v1_g58_audit_validation"] == {
        "passed": True,
        "tests_passed": 10,
    }
    assert validation["focused_v1_g58_rollup_validation"] == {
        "passed": True,
        "tests_passed": 301,
    }
...
def test_runtime_readiness_rollup_g58_doc_contains_next_lane_and_boundaries() -> None:
    fixture = _load_fixture()
    text = (REPO_ROOT / fixture["documents"]["rollup"]).read_text(encoding="utf-8")

    assert "# V1 Runtime Readiness Rollup Through G58" in text
    assert "Built-in provider SDK client authority contract metadata: `CANDIDATE_ONLY`" in text
    assert "Built-in provider SDK client implementation: `NOT_APPROVED`" in text
