"""Static checks for the V1-G36 bounded integration design request."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g36_bounded_consumer_integration_design_approval_request.json"
)
...
def test_v1_g36_request_docs_and_fixture_exist() -> None:
    fixture = _load_fixture()

    assert fixture["gate_id"] == "v1_g36_bounded_consumer_integration_design_approval_request"
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == (
        "prepare-v1-g36-bounded-consumer-integration-design-approval-request"
    )
...
def test_v1_g36_has_no_implementation_approval_yet() -> None:
    fixture = _load_fixture()
    decision = fixture["decision_record"]

    assert fixture["implementation_approved"] is False
    assert fixture["operator_approval_recorded"] is False
    assert fixture["bounded_consumer_integration_design_approved"] is False
    assert fixture["bounded_consumer_integration_design_added"] is False
...
def test_v1_g36_exact_decision_options_are_locked() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["required_approval_wording"].startswith(
        "I explicitly approve V1-G36 implementation"
    )
...
def test_v1_g36_design_scope_is_metadata_only() -> None:
    scope = _load_fixture()["design_scope_if_operator_says_yes"]

    assert scope["metadata_design_only"] is True
...
def test_v1_g36_approved_lima_file_scope_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g36_prior_evidence_refs_exist() -> None:
    fixture = _load_fixture()
...
def test_v1_g36_forbidden_runtime_and_external_boundaries_remain_false() -> None:
    fixture = _load_fixture()
...
def test_v1_g36_docs_contain_design_boundary_language() -> None:
    fixture = _load_fixture()
    ...
    assert "bounded consumer integration design" in approval_text
    assert "must remain design metadata only" in approval_text
    assert "Consumer integration approved: no" in approval_text
    assert "Recorded choice: none" in decision_text
    assert "Recorded choice: Approve-V1-G36" in decision_text
    assert "Implementation must not start until `Approve-V1-G36`" in preflight_text
