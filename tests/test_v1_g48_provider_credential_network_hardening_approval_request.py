"""Static checks for the V1-G48 provider credential/network hardening request."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g48_provider_credential_network_hardening_approval_request.json"
)
...
def test_v1_g48_request_docs_and_fixture_exist() -> None:
    fixture = _load_fixture()

    assert fixture["gate_id"] == (
        "v1_g48_provider_credential_network_hardening_approval_request"
    )
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == (
        "prepare-v1-g48-provider-credential-network-hardening-approval-request"
    )
...
def test_v1_g48_has_no_implementation_approval_yet() -> None:
    fixture = _load_fixture()
    decision = fixture["decision_record"]

    assert fixture["implementation_approved"] is False
    assert fixture["operator_approval_recorded"] is False
    assert fixture["provider_credential_network_hardening_approved"] is False
    assert fixture["provider_credential_network_hardening_added"] is False
...
def test_v1_g48_exact_decision_options_are_locked() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["required_approval_wording"].startswith(
        "I explicitly approve V1-G48 implementation"
    )
...
def test_v1_g48_hardening_target_is_metadata_only() -> None:
    target = _load_fixture()[
        "provider_credential_network_hardening_target_if_operator_says_yes"
    ]
...
def test_v1_g48_approved_file_scope_if_approved_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g48_forbidden_boundaries_remain_false_in_request() -> None:
    fixture = _load_fixture()
...
def test_v1_g48_docs_contain_boundary_language() -> None:
    fixture = _load_fixture()
    ...
    assert "Credential values allowed: no" in approval_text
    ...
    assert "Template for `Approve-V1-G48`" in decision_text
    assert "Implementation must not start until `Approve-V1-G48`" in preflight_text
