"""Tests for the approved V1-G58 provider SDK client authority contract slice."""
...
def test_v1_g58_fixture_records_approved_scope_and_status() -> None:
    fixture = _load_fixture()

    assert fixture["packet_set_id"] == "v1_g58_built_in_provider_sdk_client_authority_contract"
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == "v1-g58-built-in-provider-sdk-client-authority-contract"
    ...
    assert fixture["required_approval_wording"] == (
        "I explicitly approve V1-G58 implementation of the LIMA-side built-in "
        "provider SDK client authority contract metadata slice, limited to the "
        "file scope, behavior scope, tests, rollback plan, and stop conditions "
        "in docs/V1_G58_BUILT_IN_PROVIDER_SDK_CLIENT_AUTHORITY_CONTRACT_APPROVAL_REQUEST.md."
    )
    assert fixture["built_in_provider_sdk_client_authority_contract_approved"] is True
    assert fixture["built_in_provider_sdk_client_authority_contract_added"] is True
    assert fixture["built_in_provider_sdk_client_authority_contract_result"] == (
...
def test_v1_g58_lima_file_scope_is_exact_and_runtime_unchanged() -> None:
    fixture = _load_fixture()
...
def test_v1_g58_authority_contract_requirements_are_locked() -> None:
    requirements = _load_fixture()["authority_contract_requirements"]

    assert requirements == {
        "guardian_gate_required": True,
        "operator_approval_linkage_required": True,
        "provider_capability_declaration_metadata_required": True,
        ...
        "links_v1_g48_v1_g53_v1_g54_v1_g55_v1_g56_v1_g57_evidence_required": True,
        "audit_evidence_metadata_is_not_execution_authority": True,
        "approval_metadata_is_not_broad_execution_authority": True,
    }


def test_v1_g58_sdk_client_implementation_and_consumer_runtime_remain_blocked() -> None:
    fixture = _load_fixture()
...
def test_v1_g58_forbidden_boundaries_remain_false() -> None:
    fixture = _load_fixture()
...
def test_v1_g58_future_gates_are_required_before_expansion() -> None:
    fixture = _load_fixture()
...
def test_v1_g58_accepted_evidence_refs_exist() -> None:
    for relative_path in _load_fixture()["accepted_evidence_refs"]:
        assert (REPO_ROOT / relative_path).exists(), relative_path


def test_v1_g58_decision_packet_records_exact_approval() -> None:
    decision_text = (
        REPO_ROOT
        / "docs"
        / "V1_G58_BUILT_IN_PROVIDER_SDK_CLIENT_AUTHORITY_CONTRACT_OPERATOR_DECISION_PACKET.md"
...
def test_v1_g58_docs_contain_boundary_language() -> None:
    implementation_text = (
        REPO_ROOT
        / "docs"
        / "V1_G58_BUILT_IN_PROVIDER_SDK_CLIENT_AUTHORITY_CONTRACT.md"
    ).read_text(encoding="utf-8")
    closeout_text = (
        REPO_ROOT
        / "docs"
        / "V1_G58_BUILT_IN_PROVIDER_SDK_CLIENT_AUTHORITY_CONTRACT_CLOSEOUT.md"
    ).read_text(encoding="utf-8")

    assert "metadata-only built-in provider SDK client authority contract" in implementation_text
    assert "No `lima/` runtime file" in implementation_text
    assert "Built-in provider SDK client implementation added: no" in implementation_text
    ...
    assert "V1-G58 is complete" in closeout_text
    assert "Product readiness claimed: no" in closeout_text
    assert "Final public API freeze claimed: no" in closeout_text


def test_v1_g58_fixture_and_docs_do_not_include_sensitive_markers() -> None:
    fixture = _load_fixture()
