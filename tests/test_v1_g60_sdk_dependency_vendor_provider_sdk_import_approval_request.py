"""Static checks for the V1-G60 SDK dependency/vendor import approval request."""
...
def test_v1_g60_request_docs_and_fixture_exist() -> None:
    fixture = _load_fixture()

    assert fixture["gate_id"] == (
        "v1_g60_sdk_dependency_vendor_provider_sdk_import_approval_request"
    )
...
def test_v1_g60_has_no_implementation_approval_yet() -> None:
    fixture = _load_fixture()
    decision = fixture["decision_record"]

    assert fixture["implementation_approved"] is False
    assert fixture["operator_approval_recorded"] is False
    assert fixture["sdk_dependency_addition_vendor_provider_sdk_import_approved"] is False
...
def test_v1_g60_exact_decision_options_are_locked() -> None:
    fixture = _load_fixture()
...
def test_v1_g60_target_if_approved_is_bounded_dependency_and_import_only() -> None:
    target = _load_fixture()[
        "sdk_dependency_vendor_provider_sdk_import_target_if_operator_says_yes"
    ]

    assert target["sdk_dependency_addition_allowed"] is True
    assert target["dependency_manifest_edit_allowed"] is True
    assert target["approved_dependency_manifest"] == "pyproject.toml"
    assert target["lockfile_edit_allowed"] is False
    assert target["vendor_provider_sdk_importability_proof_allowed"] is True
    assert target["vendor_provider_sdk_import_runtime_wiring_allowed"] is False
    ...
    assert target["dependency_name_version_constraint_metadata_required"] is True
...
def test_v1_g60_target_forbidden_authorities_are_false() -> None:
    target = _load_fixture()[
        "sdk_dependency_vendor_provider_sdk_import_target_if_operator_says_yes"
    ]
...
def test_v1_g60_approved_file_scope_if_approved_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g60_prior_evidence_refs_exist() -> None:
    for relative_path in _load_fixture()["required_prior_evidence_refs"]:
        assert (REPO_ROOT / relative_path).exists(), relative_path


def test_v1_g60_forbidden_boundaries_remain_false_in_request() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["supply_chain_review_metadata_required"] is True
    assert fixture["license_security_posture_metadata_required"] is True
    assert fixture["dependency_name_version_constraint_metadata_required"] is True
    assert fixture["vendor_import_declaration_metadata_required"] is True


def test_v1_g60_docs_contain_request_only_boundary_language() -> None:
    fixture = _load_fixture()
    ...
    assert "Implementation must not start until `Approve-V1-G60`" in preflight_text
    assert "No `lima/` runtime files may be changed." in work_order_text


def test_v1_g60_docs_and_fixture_do_not_include_sensitive_markers() -> None:
    fixture = _load_fixture()
