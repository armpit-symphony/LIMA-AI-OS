"""Tests for the approved V1-G41 consumer integration implementation slice."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g41_consumer_integration_implementation.json"
)
...
def _records() -> list[dict[str, Any]]:
    records = _load_fixture()["consumer_implementation_records"]
    assert isinstance(records, list)
    return records


def test_v1_g41_fixture_records_approved_scope_and_candidate_status() -> None:
    fixture = _load_fixture()

    assert fixture["packet_set_id"] == "v1_g41_consumer_integration_implementation"
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == "v1-g41-consumer-integration-implementation"
    assert fixture["operator_decision"] == "Approve-V1-G41"
    assert fixture["approved_scope"] == "consumer_integration_implementation_slice"
    ...
    assert fixture["static_consumer_integration_implementation_evidence_added"] is True
...
def test_v1_g41_lima_file_scope_is_exact_and_runtime_free() -> None:
    fixture = _load_fixture()
...
def test_v1_g41_contains_exactly_two_consumer_implementation_records() -> None:
    records = _records()
...
def test_v1_g41_records_saved_consumer_commits_and_source_refs() -> None:
    expected = {
        "sparkbot": (
            "sparkpit-labs/Sparkbot",
            "0ce38cbc7ad9253956fc91082302d49be46e9904",
        ...
        "arc_bot": (
            ...
            "2f89a85997cc945d06f74e464489a6a75406ddf7",
    ...
    for record in _records():
        repository, commit_sha, shell_ref, import_ref, edit_ref, fixture_ref = expected[
            record["consumer_packet_family"]
        ]

        assert record["consumer_implementation_record_id"].startswith(
            "consumer-integration-implementation:v1-g41:"
        )
        assert record["consumer_repository"] == repository
        assert record["consumer_branch"] == "v1-g41-consumer-integration-implementation"
        ...
        assert record["source_g41_consumer_fixture_ref"] == fixture_ref


def test_v1_g41_implementation_result_and_remaining_gaps_are_locked() -> None:
    expected_gaps = [
    ...
    for record in _records():
        assert record["consumer_integration_implementation_result"] == (
            "static_consumer_integration_implementation_evidence_created"
        )
        assert record["consumer_integration_implementation_approved"] is True
        assert record["consumer_integration_implementation_added"] is True
        assert record["static_implementation_fixture_added"] is True
        assert record["static_implementation_test_added"] is True
        assert record["bounded_consumer_integration_implementation_evidence_only"] is True
        assert record["static_import_smoke_fixture_reference_validated"] is True
        assert record["static_candidate_fixture_reference_validated"] is True
        assert record["docs_tests_fixtures_only"] is True
        assert record["remaining_gaps"] == expected_gaps
        assert record["runtime_consumer_integration_added"] is False
...
def test_v1_g41_links_required_prior_evidence_documents() -> None:
    fixture = _load_fixture()
...
def test_v1_g41_runtime_patch_and_external_boundaries_remain_false() -> None:
    fixture = _load_fixture()
...
def test_v1_g41_rollback_metadata_is_exact_and_reversible() -> None:
    fixture = _load_fixture()
    ...
    for record in _records():
        ...
        assert rollback["rollback_ref"].startswith("rollback:v1-g41:")
...
def test_v1_g41_required_confirmations_are_true() -> None:
    for record in _records():
        ...
        assert (
            confirmations[
                "consumer_integration_implementation_approval_recorded_confirmation"
            ]
            is True
        )
...
def test_v1_g41_output_does_not_include_patch_bodies_imports_or_sensitive_markers() -> None:
...
def test_v1_g41_docs_contain_boundary_language() -> None:
    implementation_text = (
        REPO_ROOT / "docs" / "V1_G41_CONSUMER_INTEGRATION_IMPLEMENTATION.md"
    ).read_text(encoding="utf-8")
    closeout_text = (
        REPO_ROOT / "docs" / "V1_G41_CONSUMER_INTEGRATION_IMPLEMENTATION_CLOSEOUT.md"
    ).read_text(encoding="utf-8")

    assert "approved bounded consumer integration implementation evidence slice" in implementation_text
    ...
    assert "V1-G41 is complete" in closeout_text
