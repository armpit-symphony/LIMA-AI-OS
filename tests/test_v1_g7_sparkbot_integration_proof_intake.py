"""Static checks for the V1-G7 Sparkbot integration proof intake."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g7_sparkbot_integration_proof_intake.json"
)
DOCS = {
    "intake": REPO_ROOT / "docs" / "V1_G7_SPARKBOT_INTEGRATION_PROOF_INTAKE.md",
    "audit": REPO_ROOT / "docs" / "V1_G7_SPARKBOT_INTEGRATION_PROOF_INTAKE_AUDIT.md",
    "closeout": REPO_ROOT / "docs" / "V1_G7_SPARKBOT_INTEGRATION_PROOF_INTAKE_CLOSEOUT.md",
}
...
def test_sparkbot_intake_summary_and_docs_exist() -> None:
    ...
    assert summary["shell_repo"] == "Sparkbot"
    assert summary["api_status"] == "CANDIDATE_ONLY"
    assert summary["branch"] == "intake-v1-g7-sparkbot-integration-proof-packet"
    assert summary["source_branch"] == "intake-v1-g7-sparkbot-shell-integration-proof-packet"
    assert summary["sparkbot_branch"] == "v1-g7-sparkbot-integration-proof-packet"
    assert summary["sparkbot_commit"] == "0bb99352a9b62cf1dc35e075c9f3a08054b6bef1"
    assert summary["sparkbot_shell_packet_accepted"] is True
    assert summary["sparkbot_packet_received"] is True
    assert summary["sparkbot_packet_accepted"] is True
    assert summary["arc_bot_shell_packet_received"] is False
    assert summary["arc_bot_shell_packet_accepted"] is False
...
def test_sparkbot_intake_preserves_boundaries() -> None:
    summary = _load_json(FIXTURE_PATH)
    for key in (
...
def test_sparkbot_intake_state_status_and_haptics() -> None:
    summary = _load_json(FIXTURE_PATH)
    ...
    assert summary["docs_fixture_only_shell_response_states"] == ["deferred"]
    ...
    assert summary["destructive_edit_delete_runtime_present"] is True


def test_sparkbot_intake_accepts_and_rejects_correct_claims() -> None:
    summary = _load_json(FIXTURE_PATH)
    ...
    assert "Sparkbot source-backed approval and policy decision reference evidence" in accepted
    assert "Sparkbot source-backed provider/model routing reference evidence" in accepted
    assert "Sparkbot destructive edit/delete approval posture" in accepted
    ...
    assert "arc_bot_shell_v1_g7_packet_not_accepted" in blockers
    assert "arc_bot_shell_lima_intake_audit_not_complete" in blockers
    assert "consolidated_v1_g7_closeout_not_complete" in blockers


def test_sparkbot_intake_docs_state_static_only_verdict() -> None:
    ...
    assert "LIMA can accept this Sparkbot packet as static V1-G7 behavior-reference evidence." in intake_text
    assert "LIMA cannot treat it as live LIMA runtime parity." in intake_text
    assert "API status remains `CANDIDATE_ONLY`" in intake_text
    assert "accept_static_behavior_reference_evidence_only" in audit_text
    assert "Arc-Bot-shell: not yet delivered or audited for V1-G7." in audit_text
    assert "API status remains: `CANDIDATE_ONLY`." in closeout_text
    assert "Recommended: `V1-G7A`." in closeout_text
