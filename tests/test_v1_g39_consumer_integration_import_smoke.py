"""Tests for the approved V1-G39 consumer integration import-smoke slice."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g39_consumer_integration_import_smoke.json"
)
...
def _records() -> list[dict[str, Any]]:
    records = _load_fixture()["import_smoke_records"]
    assert isinstance(records, list)
    return records


def test_v1_g39_fixture_records_approved_scope_and_candidate_status() -> None:
    fixture = _load_fixture()

    assert fixture["packet_set_id"] == "v1_g39_consumer_integration_import_smoke"
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == "v1-g39-consumer-integration-import-smoke"
    assert fixture["operator_decision"] == "Approve-V1-G39"
    assert fixture["approved_scope"] == "consumer_integration_import_smoke_slice"
    ...
    assert fixture["static_consumer_integration_import_smoke_added"] is True
...
def test_v1_g39_lima_file_scope_is_exact_and_runtime_free() -> None:
    fixture = _load_fixture()
...
def test_v1_g39_contains_exactly_two_import_smoke_records() -> None:
    records = _records()
...
def test_v1_g39_records_saved_consumer_commits_and_g38_refs() -> None:
    expected = {
        "sparkbot": (
            "sparkpit-labs/Sparkbot",
            "b4fd57bbbbb835098598e1d602a8254c0438ade2",
        ...
        "arc_bot": (
            ...
            "772c0c7a2668d562f369fc5b13afde0dcb1e0f99",
    ...
    for record in _records():
        repository, commit_sha, repository_edit_ref, candidate_fixture_ref = expected[
            record["consumer_packet_family"]
        ]

        assert record["import_smoke_record_id"].startswith(
            "consumer-integration-import-smoke:v1-g39:"
        )
        assert record["consumer_repository"] == repository
        assert record["consumer_branch"] == "v1-g39-consumer-integration-import-smoke"
        assert record["consumer_commit_sha"] == commit_sha
        assert len(record["consumer_commit_sha"]) == 40
        assert record["source_repository_edit_record_ref"] == repository_edit_ref
        assert record["source_g38_candidate_fixture_ref"] == candidate_fixture_ref


def test_v1_g39_import_smoke_result_and_remaining_gaps_are_locked() -> None:
    expected_gaps = [
    ...
    for record in _records():
        assert record["consumer_integration_import_smoke_result"] == (
            "static_consumer_integration_import_smoke_evidence_created"
        )
        assert record["consumer_integration_import_smoke_approved"] is True
        assert record["consumer_integration_import_smoke_added"] is True
        assert record["static_import_smoke_fixture_added"] is True
        assert record["static_import_smoke_test_added"] is True
        assert record["static_candidate_fixture_reference_validated"] is True
        assert record["static_import_reference_smoke_only"] is True
...
def test_v1_g39_links_required_prior_evidence_documents() -> None:
    fixture = _load_fixture()
...
def test_v1_g39_runtime_patch_and_external_boundaries_remain_false() -> None:
    fixture = _load_fixture()
...
def test_v1_g39_rollback_metadata_is_exact_and_reversible() -> None:
    fixture = _load_fixture()
    ...
    for record in _records():
        ...
        assert rollback["rollback_ref"].startswith("rollback:v1-g39:")
...
def test_v1_g39_required_confirmations_are_true() -> None:
    for record in _records():
        ...
        assert (
            confirmations[
                "consumer_integration_import_smoke_approval_recorded_confirmation"
            ]
            is True
        )
...
def test_v1_g39_output_does_not_include_patch_bodies_imports_or_sensitive_markers() -> None:
...
def test_v1_g39_docs_contain_boundary_language() -> None:
    implementation_text = (
        REPO_ROOT / "docs" / "V1_G39_CONSUMER_INTEGRATION_IMPORT_SMOKE.md"
    ).read_text(encoding="utf-8")
    closeout_text = (
        REPO_ROOT / "docs" / "V1_G39_CONSUMER_INTEGRATION_IMPORT_SMOKE_CLOSEOUT.md"
    ).read_text(encoding="utf-8")

    assert "approved consumer integration import-smoke slice" in implementation_text
    ...
    assert "V1-G39 is complete" in closeout_text
