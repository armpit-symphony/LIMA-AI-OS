"""Static checks for Phase 17.0 acceptance test audit charter."""
...
PHASE_DOC_PATH = REPO_ROOT / "docs" / "PHASE_17_0_PHASE_16_ACCEPTANCE_TEST_AUDIT_CHARTER.md"
PHASE_FIXTURE_PATH = (
    REPO_ROOT
    / "tests"
    / "fixtures"
    / "runtime_extraction"
    / "phase_17_0_phase_16_acceptance_test_audit_charter.json"
)
...
def test_phase_is_docs_tests_fixtures_only() -> None:
    ...
    assert fixture["phase"] == "17.0"
    assert fixture["docs_tests_fixtures_only"] is True
    assert fixture["runtime_code_modified"] is False


def test_phase_sixteen_audit_scope_is_complete() -> None:
    ...
    assert scope == {
        ...
        "phase_16_5_phase_16_test_only_acceptance_archive_closeout",
    }


def test_phase_eighteen_options_are_listed() -> None:
    options = set(_load_json(PHASE_FIXTURE_PATH)["phase_18_options"])
    assert "no_code_design_lane_for_next_narrow_runtime_slice" in options
    assert "test_only_regression_hardening_lane" in options
...
def test_phase_document_preserves_boundaries_and_gate() -> None:
    ...
    assert "Phase 17.0 does not approve Phase 18" in phase_doc
...
def test_boundary_results_show_no_forbidden_changes() -> None:
    ...
    assert boundary["phase_18_approved"] is False


def test_no_phase_seventeen_zero_files_exist_under_lima_or_tests_support() -> None:
    assert not list((REPO_ROOT / "lima").rglob("*phase_17_0*"))
    assert not list((REPO_ROOT / "tests" / "support").rglob("*phase_17_0*"))
