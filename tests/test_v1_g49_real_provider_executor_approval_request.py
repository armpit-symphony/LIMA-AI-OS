"""Static checks for the V1-G49 real provider executor request."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g49_real_provider_executor_approval_request.json"
)
...
def test_v1_g49_request_docs_and_fixture_exist() -> None:
    fixture = _load_fixture()

    assert fixture["gate_id"] == "v1_g49_real_provider_executor_approval_request"
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == "prepare-v1-g49-real-provider-executor-approval-request"

    for relative_path in fixture["documents"].values():
        assert (REPO_ROOT / relative_path).exists()


def test_v1_g49_has_no_implementation_approval_yet() -> None:
    fixture = _load_fixture()
    decision = fixture["decision_record"]

    assert fixture["implementation_approved"] is False
    assert fixture["operator_approval_recorded"] is False
    assert fixture["real_provider_executor_authority_design_approved"] is False
    assert fixture["real_provider_executor_authority_design_added"] is False
...
def test_v1_g49_exact_decision_options_are_locked() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["required_approval_wording"].startswith(
        "I explicitly approve V1-G49 implementation"
    )
    assert fixture["proposed_implementation_branch"] == "v1-g49-real-provider-executor"


def test_v1_g49_executor_target_is_metadata_only() -> None:
    target = _load_fixture()["real_provider_executor_target_if_operator_says_yes"]

    assert target["executor_authority_metadata_only"] is True
...
def test_v1_g49_approved_file_scope_if_approved_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g49_forbidden_boundaries_remain_false_in_request() -> None:
    fixture = _load_fixture()
...
def test_v1_g49_docs_contain_boundary_language() -> None:
    fixture = _load_fixture()
    ...
    assert "Template for `Approve-V1-G49`" in decision_text
    assert "Implementation must not start until `Approve-V1-G49`" in preflight_text
