"""Phase 33 hardening audit charter tests for Phase 34.0."""
...
PHASE_DOC_PATH = (
    REPO_ROOT / "docs" / "PHASE_34_0_PHASE_33_HARDENING_AUDIT_CHARTER.md"
)
PHASE_FIXTURE_PATH = (
    REPO_ROOT
    / "tests"
    / "fixtures"
    / "runtime_extraction"
    / "phase_34_0_phase_33_hardening_audit_charter.json"
)
...
def test_phase_34_0_opens_audit_archive_only_lane() -> None:
    fixture = _load_json(PHASE_FIXTURE_PATH)
    phase_doc = PHASE_DOC_PATH.read_text(encoding="utf-8")
    assert fixture["phase"] == "34.0"
    assert fixture["phase_33_audit_result"] == "PASS"
    assert fixture["docs_tests_fixtures_only"] is True
    assert fixture["runtime_implementation_approved"] is False
    assert fixture["runtime_files_changed_in_phase_34"] == []
    assert "does not implement runtime behavior" in phase_doc


def test_phase_33_audit_verified_no_runtime_or_support_changes() -> None:
    verified = _load_json(PHASE_FIXTURE_PATH)["phase_33_verified"]
...
def test_phase_33_audit_verified_no_forbidden_behavior() -> None:
    verified = _load_json(PHASE_FIXTURE_PATH)["phase_33_verified"]
...
def test_phase_33_validation_is_recorded_as_passing() -> None:
    verified = _load_json(PHASE_FIXTURE_PATH)["phase_33_verified"]
    assert verified["phase_33_targeted_tests_passed"] is True
    ...
    assert verified["runtime_state_scan_only_defensive_deny_wording"] is True


def test_no_phase_34_0_files_exist_under_lima_or_tests_support() -> None:
    assert not list((REPO_ROOT / "lima").rglob("*phase_34_0*"))
    assert not list((REPO_ROOT / "tests" / "support").rglob("*phase_34_0*"))
