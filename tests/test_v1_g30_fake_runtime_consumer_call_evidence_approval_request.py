"""Static checks for the V1-G30 fake-runtime consumer call evidence request."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g30_fake_runtime_consumer_call_evidence_approval_request.json"
)
...
def test_v1_g30_request_docs_and_fixture_exist() -> None:
    fixture = _load_fixture()

    assert fixture["gate_id"] == "v1_g30_fake_runtime_consumer_call_evidence_approval_request"
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == "prepare-v1-g30-fake-runtime-consumer-call-evidence-approval-request"

    for relative_path in fixture["documents"].values():
        assert (REPO_ROOT / relative_path).exists()


def test_v1_g30_has_no_implementation_approval_yet() -> None:
    fixture = _load_fixture()
    decision = fixture["decision_record"]

    assert fixture["implementation_approved"] is False
    assert fixture["operator_approval_recorded"] is False
    assert fixture["fake_runtime_consumer_call_evidence_approved"] is False
    assert fixture["fake_runtime_consumer_call_evidence_added"] is False
...
def test_v1_g30_exact_decision_options_are_locked() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["required_approval_wording"].startswith(
        "I explicitly approve V1-G30 implementation"
    )
...
def test_v1_g30_fake_runtime_adapter_symbols_are_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g30_fake_runtime_scope_is_metadata_only() -> None:
    scope = _load_fixture()["fake_runtime_scope_if_operator_says_yes"]

    assert scope["metadata_only"] is True
    assert scope["fake_runtime_only"] is True
    assert scope["fake_call_envelopes_allowed"] is True
...
def test_v1_g30_approved_file_scope_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g30_prior_evidence_refs_exist() -> None:
    fixture = _load_fixture()
...
def test_v1_g30_forbidden_runtime_and_external_boundaries_remain_false() -> None:
    fixture = _load_fixture()
...
def test_v1_g30_docs_contain_fake_runtime_boundary_language() -> None:
    fixture = _load_fixture()
    ...
    assert "fake-runtime consumer call evidence" in approval_text
    ...
    assert "Recorded choice: Approve-V1-G30" in decision_text
    assert "Implementation must not start until `Approve-V1-G30`" in preflight_text
