"""Static checks for the V1-G42 shell wiring implementation request."""
...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "runtime_extraction"
    / "v1_g42_shell_wiring_implementation_approval_request.json"
)
...
def test_v1_g42_request_docs_and_fixture_exist() -> None:
    fixture = _load_fixture()

    assert fixture["gate_id"] == "v1_g42_shell_wiring_implementation_approval_request"
    assert fixture["api_status"] == "CANDIDATE_ONLY"
    assert fixture["branch"] == "prepare-v1-g42-shell-wiring-implementation-approval-request"

    for relative_path in fixture["documents"].values():
        assert (REPO_ROOT / relative_path).exists()


def test_v1_g42_has_no_implementation_approval_yet() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["shell_wiring_implementation_added"] is False
    assert fixture["runtime_shell_wiring_execution_approved"] is False
...
def test_v1_g42_exact_decision_options_are_locked() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["required_approval_wording"].startswith(
        "I explicitly approve V1-G42 implementation"
    )
...
def test_v1_g42_edit_scope_is_exact_and_limited() -> None:
    ...
    assert scope["bounded_shell_wiring_implementation_evidence_allowed"] is True
...
def test_v1_g42_approved_lima_file_scope_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g42_approved_consumer_file_scope_is_exact() -> None:
    fixture = _load_fixture()
...
def test_v1_g42_prior_evidence_refs_exist() -> None:
    fixture = _load_fixture()
...
def test_v1_g42_forbidden_runtime_and_external_boundaries_remain_false() -> None:
    fixture = _load_fixture()
...
def test_v1_g42_docs_contain_shell_wiring_boundary_language() -> None:
    fixture = _load_fixture()
    ...
    assert "shell wiring implementation" in approval_text
    assert "No `lima/` runtime files" in approval_text
    assert "Shell wiring implementation approved: no" in approval_text
    assert "Runtime shell wiring execution approved: no" in approval_text
    assert "Recorded choice: none" in decision_text
    assert "Recorded choice: Approve-V1-G42" in decision_text
    assert "Implementation must not start until `Approve-V1-G42`" in preflight_text
