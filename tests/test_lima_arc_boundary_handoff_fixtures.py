...
REQUIRED_EVIDENCE_FIELDS = {
    ...
    "normalized_office_task_request_fixture",
    "dry_run_execution_result_sample",
    "non_execution_invariant_checklist",
    "proof_no_raw_office_task_text_sent_to_lima",
    "proof_no_customer_record_payload_sent_to_lima",
...
def _iter_cases() -> list[Mapping[str, Any]]:
    ...
    assert fixture["fixture_scope"] == "synthetic_arc_boundary_handoff_fixture_only"
...
def _build_kernel_request(normalized_request: Mapping[str, Any]) -> KernelRequest:
    ...
    return KernelRequest(
        ...
        metadata={
            ...
            "arc_boundary_handoff_fixture": True,
            "synthetic": True,
...
def test_arc_handoff_fixture_declares_no_repo_or_runtime_wiring() -> None:
    fixture = _load_fixture()
    ...
    assert fixture["arc_integration_implemented"] is False
    assert fixture["production_readiness_claimed"] is False


def test_arc_handoff_checklist_shape_is_archive_ready() -> None:
    fixture = _load_fixture()

    assert set(fixture["required_arc_side_evidence"]) == REQUIRED_EVIDENCE_FIELDS
    proof = fixture["allowed_future_arc_proof"]
    assert proof["arc_owned_branch"] == "arc-lima-dry-run-boundary-proof"
    assert proof["normalized_office_task_metadata_only"] is True
    assert proof["dry_run_only"] is True
    assert proof["must_not_wire_production_route"] is True
    assert proof["must_not_trigger_scheduler_or_background_worker"] is True
...
def test_arc_handoff_fixture_values_are_redacted_and_synthetic() -> None:
    fixture = _load_fixture()
...
def test_arc_handoff_fixture_maps_to_kernel_request() -> None:
    for case in _iter_cases():
        request = _build_kernel_request(case["request"])

        assert isinstance(request, KernelRequest)
        assert request.shell_context["shell_type"] == "arc"
        assert request.normalized_intent["execution_mode"] == "dry_run"
        assert request.metadata["arc_boundary_handoff_fixture"] is True
...
def test_arc_handoff_fixtures_evaluate_as_dry_run_only() -> None:
    for case in _iter_cases():
        request = _build_kernel_request(case["request"])
...
def test_arc_handoff_simulated_discovery_surface_is_synthetic_only() -> None:
    case = next(
        fixture_case
        for fixture_case in _iter_cases()
        if fixture_case["fixture_id"] == "arc-handoff-simulated-discovery-preview"
    )
...
def test_arc_handoff_test_file_does_not_import_arc_sparkbot_or_live_surfaces() -> None:
