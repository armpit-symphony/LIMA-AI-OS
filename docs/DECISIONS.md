...
## ADR-0250: Phase 29 Should Move To No-Code Design Review

Status: Accepted

Decision:

Phase 28.2 finds no specific documented risk requiring another automatic preservation pause and recommends Phase 29 as a docs/tests/fixtures-only no-code design review for the next narrow runtime slice.

Context:

The current runtime/test state is stable and no concrete immediate test-only hardening gap was identified. Continued pause remains safe, but a sharper next decision is appropriate.

Consequences:

- Phase 28.2 changes no runtime files.
- Phase 28.2 changes no `tests/support/` files.
...
## ADR-0266: Phase 32 Should Be No-Code Runtime Slice Design Review

Status: Accepted

Decision:

Phase 31.3 recommends Phase 32 as a docs/tests/fixtures-only design review for the next narrow runtime slice.

Context:

Phase 30 is stable and bounded, but Phase 31 found no reason to proceed directly into another implementation. The next safe step is design review and explicit approval gating.

Consequences:

- Phase 31.3 changes no runtime files.
- Phase 31.3 changes no `tests/support/` files.
- Phase 32 is not runtime implementation approval.
- Phase 5 HumanInput runtime bridge remains gated.

## ADR-0267: Phase 31 Archives The Phase 30 Runtime Slice Audit

Status: Accepted

Decision:

Phase 31.4 archives Phase 31 as the completed docs/tests/fixtures-only audit/archive for the Phase 30 read-only runtime state inspection slice.

Context:

Phase 31 audited the first runtime state inspection slice, found no blocking safety regression, confirmed no Phase 31 runtime files changed, and recommended Phase 32 as no-code design review.

Consequences:

- Phase 31.4 changes no runtime files.
- Phase 31.4 changes no `tests/support/` files.
...
Phase 28.4 archives Phase 28 as a completed docs/tests/fixtures-only preservation status review and preserves Phase 29 as a gated no-code design review.

Context:

Phase 28 confirmed stable state, found no concrete immediate test-only hardening gap, found no specific risk requiring another automatic pause, and selected no-code design review as the sharper next step.

Consequences:

- Phase 28.4 changes no runtime files.
- Phase 28.4 changes no `tests/support/` files.
...
## ADR-0331: Phase 47.2 Opens Static Acceptance-Test Checklist Readiness Review

Status: Accepted

Decision:

Phase 47.2 opens as docs/tests/fixtures-only static checklist readiness-review work over the completed Phase 47.1 static acceptance-test implementation checklist.

Context:

Phase 47.1 is complete and tagged on `main` as `phase-47.1-static-acceptance-test-implementation-checklist`. The next smallest safe step is readiness review before archive closeout, while runtime and harness implementation remain unapproved.

Consequences:

- Phase 47.2 may add readiness-review documentation, inert JSON review metadata, deterministic tests, and tracking-file updates only.
- No runtime behavior is implemented.
- No runtime test harness is created or activated.
- No actual or executable runtime bridge acceptance tests are created.
- No `lima/` changes are approved.
