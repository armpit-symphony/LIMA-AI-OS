# V1-G41 Consumer Integration Implementation Approval Request
...
This is an approval request only. It does not approve implementation, edit `lima/` runtime files, edit Sparkbot, edit Arc-Bot-shell, create consumer implementation test files, call adapter symbols, import consumer runtime modules, wire shells, call providers/models, read secrets, execute tools, invoke connectors, execute browser/network/file/device/robotics/physical-world behavior, run scheduled tasks, send external messages, persist raw sensitive content in LIMA evidence, or claim product readiness.

## Exact Approval Question

Do you explicitly approve V1-G41 implementation of the consumer integration implementation slice, limited to the file scope, behavior scope, tests, rollback plan, and stop conditions below?

Approval must be explicit before implementation begins. V1-G40, readiness rollups, decision matrices, broad product direction, or this request packet do not count as implementation approval.

## Proposed V1-G41 Objective

Implement the smallest approved bounded consumer integration implementation evidence slice.

The slice should add exact static consumer integration implementation test/fixture files to Sparkbot and Arc-Bot-shell, plus deterministic LIMA-side docs/tests/fixtures evidence. It must not edit consumer runtime/source files beyond the exact approved test/fixture paths, wire shells, call adapter symbols from LIMA implementation code, import consumer runtimes, call providers/models, access credentials, invoke connectors, use network/browser/device/robotics/physical-world behavior, or claim product readiness.
...
No `lima/` runtime files may be created, edited, removed, or renamed in V1-G41.

No Sparkbot or Arc-Bot-shell files outside the exact approved test/fixture paths above may be created, edited, removed, or renamed in V1-G41.

Any other file requires a new gate update before implementation.

## Allowed Behavior If Approved

V1-G41 may add only deterministic LIMA-side docs/tests/fixtures plus the exact approved Sparkbot and Arc-Bot-shell static test/fixture files.

Allowed if approved:

- add one LIMA-side consumer integration implementation evidence fixture
- add one focused LIMA-side consumer integration implementation evidence test
- add one Sparkbot static consumer integration implementation fixture
- add one Sparkbot static consumer integration implementation test
- add one Arc-Bot-shell static consumer integration implementation fixture
- add one Arc-Bot-shell static consumer integration implementation test
- run the approved local static consumer tests
- reference the V1-G40 shell boundary design records
- reference the V1-G39 import-smoke files and V1-G38 candidate files
- link V1-G40 shell wiring design evidence, audit, authority-chain audit, readiness rollup, and next-lane matrix
...
- enforce proof-not-shell-wiring-implementation, proof-not-live-dispatch-authority, and proof-not-product-readiness confirmations
...
- LIMA consumer integration implementation fixture records `CANDIDATE_ONLY`
...
- Sparkbot implementation evidence record exists and remains static-test-only
- Arc-Bot-shell implementation evidence record exists and remains static-test-only
...
- shell wiring implementation, provider/model, connector/browser/network, physical-world, and product-readiness gates remain blocked
...
- proof-not-shell-wiring-implementation and proof-not-live-dispatch-authority confirmations are enforced
...
Rollback must remove only the exact approved V1-G41 docs/tests/fixtures and consumer test/fixture files:
...
- file scope exceeds the approved V1-G41 files
...
If approved, create branch `v1-g41-consumer-integration-implementation` in LIMA-AI-OS, Sparkbot, and Arc-Bot-shell as needed. Implement only the exact consumer integration implementation evidence slice. Do not edit runtime files, edit unapproved consumer files, wire shells, call providers/models, access secrets, invoke connector/browser/network behavior, add physical-world behavior, persist raw sensitive content in LIMA evidence, or claim product readiness.
