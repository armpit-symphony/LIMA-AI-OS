# V1-G47 Consumer Fake-Executor Provider Model Call Smoke Approval Request
...
Do you explicitly approve V1-G47 implementation of the consumer fake-executor provider/model call smoke slice, limited to the file scope, behavior scope, tests, rollback plan, and stop conditions below?

Approval must be explicit before implementation begins. V1-G46, readiness rollups, decision matrices, broad product direction, or this request packet do not count as implementation approval.

## Proposed V1-G47 Objective

Implement the smallest consumer-facing fake-executor smoke slice for the V1-G46 public harness wrapper.

The proposed implementation would add focused Sparkbot and Arc-Bot-shell tests/fixtures that import the public V1-G46 `lima.harness` execution wrapper and call it with a fake in-process provider executor only. The slice is meant to prove that both consumer repositories can exercise the public import/call shape without live provider credentials, real network calls, connector behavior, production runtime calls, or product-readiness claims.
...
The approved future implementation must use a fake in-process provider executor only. It must not use real provider credentials, make network calls, add built-in provider SDK clients, execute fallback, invoke connectors, or claim product readiness.
...
V1-G47 may add only deterministic fake-executor smoke tests/fixtures and LIMA-side evidence docs/tests/fixtures.

Allowed if approved:

- add focused Sparkbot tests/fixtures for fake-executor V1-G46 import/call smoke
- add focused Arc-Bot-shell tests/fixtures for fake-executor V1-G46 import/call smoke
- add LIMA-side docs/tests/fixtures recording the consumer evidence
- import the approved V1-G46 public harness symbols in consumer tests
- build sanitized V1-G44 authority metadata in consumer tests
- call the V1-G46 wrapper with a fake in-process provider executor only
- prove returned evidence is sanitized
- prove no live provider credentials, real network calls, built-in provider SDKs, fallback, connectors, browser/network/device/robotics/physical-world behavior, or product-readiness claims are added
...
- Sparkbot imports the approved V1-G46 public harness symbols
- Arc-Bot-shell imports the approved V1-G46 public harness symbols
- Sparkbot calls the V1-G46 wrapper with a fake in-process provider executor only
- Arc-Bot-shell calls the V1-G46 wrapper with a fake in-process provider executor only
...
- no live provider credentials, real network calls, built-in provider SDK clients, fallback execution, connector/browser/network/device/robotics/physical-world behavior, or product-readiness claims are approved
...
Rollback must remove only the exact approved V1-G47 changes:

- remove the V1-G47 LIMA docs/tests/fixture
- remove the V1-G47 Sparkbot test/fixture
- remove the V1-G47 Arc-Bot-shell test/fixture
...
- file scope exceeds the approved V1-G47 files
...
- real provider executor invocation is added
...
If approved, create branch `v1-g47-consumer-fake-executor-provider-model-call-smoke` in LIMA-AI-OS and corresponding scoped consumer branches as needed. Implement only the exact fake-executor consumer smoke slice. Do not add live provider credentials, real network calls, built-in provider SDK clients, fallback, connectors, physical-world behavior, or product readiness claims.
