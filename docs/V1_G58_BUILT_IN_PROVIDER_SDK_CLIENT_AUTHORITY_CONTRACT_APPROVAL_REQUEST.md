# V1-G58 Built-In Provider SDK Client Authority Contract Approval Request
...
This is an approval request only. It does not approve implementation, edit `lima/` runtime files, edit public API exports, edit Sparkbot, edit Arc-Bot-shell, add built-in provider SDK clients, add SDK dependencies, import vendor SDKs, implement direct provider SDK code, resolve provider endpoints, add DNS/HTTP/socket/network clients, make network calls, perform direct provider egress, read secrets, access credential values, access provider tokens or API keys, change provider configuration, execute fallback, invoke connectors, execute browser/network/file/device/robotics/physical-world behavior, run scheduled tasks, send external messages, or claim product readiness.

## Exact Approval Question

Do you explicitly approve V1-G58 implementation of the LIMA-side built-in provider SDK client authority contract metadata slice, limited to the file scope, behavior scope, tests, rollback plan, and stop conditions below?

Approval must be explicit before implementation begins. V1-G57, readiness rollups, decision matrices, broad product direction, or this request packet do not count as implementation approval.

## Proposed V1-G58 Objective

Implement the smallest LIMA-side metadata-only built-in provider SDK client authority contract slice after V1-G57.

The proposed implementation would add docs/tests/fixtures defining the authority contract that must be satisfied before any future built-in provider SDK client implementation can be considered. It is intended to define the minimum metadata expectations around Guardian gating, operator approval, provider capability declaration, SDK dependency declaration, credential-reference policy, network-policy-reference policy, endpoint authority, redaction, denial-by-default posture, audit evidence, rollback, and stop conditions.

The approved future implementation must remain contract metadata only. It must not add built-in provider SDK clients, SDK dependencies, vendor provider SDK imports, direct provider SDK implementation, LIMA-owned endpoint resolution execution, LIMA-owned DNS/HTTP/socket/network clients, LIMA-owned network calls, direct provider egress, ambient secret lookup, secret lookup, credential-value access, provider token/API key access, provider configuration changes, fallback execution, consumer production runtime integration, connector/browser/network/file/device/robotics/physical-world behavior, or product-readiness claims.
...
V1-G58 may add only LIMA-side built-in provider SDK client authority contract docs/tests/fixtures.

Allowed if approved:

- record metadata-only built-in provider SDK client authority contract criteria
- require linkage to V1-G48, V1-G53, V1-G54, V1-G55, V1-G56, and V1-G57 evidence
- require Guardian gate and explicit operator approval linkage before any later built-in SDK client implementation
- require sanitized evidence refs only
- require provider capability declaration metadata
...
- require denial-by-default posture for unapproved built-in SDK client behavior
...
- prove no built-in provider SDK clients, SDK dependencies, vendor SDK imports, direct provider SDK implementation, LIMA-owned endpoint resolution, LIMA-owned DNS/HTTP/socket/network clients, LIMA-owned network calls, LIMA-owned direct provider egress, secret lookup, credential-value access, provider token/API key access, provider configuration changes, fallback, connectors, browser/network/device/robotics/physical-world behavior, consumer production runtime integration, or product-readiness claims are added
...
- built-in provider SDK client authority contract remains metadata-only
- built-in provider SDK client implementation remains blocked until later explicit approval
...
- provider capability declaration metadata is required
...
- LIMA focused V1-G57 implementation tests
- LIMA focused V1-G57 audit tests
- LIMA focused V1 runtime readiness rollup through G57 tests
- LIMA focused V1 post-G57 next-lane decision matrix tests
...
Rollback must remove only the exact approved V1-G58 changes:

- remove `docs/V1_G58_BUILT_IN_PROVIDER_SDK_CLIENT_AUTHORITY_CONTRACT.md`
- remove `docs/V1_G58_BUILT_IN_PROVIDER_SDK_CLIENT_AUTHORITY_CONTRACT_CLOSEOUT.md`
- remove `tests/fixtures/runtime_extraction/v1_g58_built_in_provider_sdk_client_authority_contract.json`
- remove `tests/test_v1_g58_built_in_provider_sdk_client_authority_contract.py`
...
- file scope exceeds the approved V1-G58 files
...
If approved, create branch `v1-g58-built-in-provider-sdk-client-authority-contract` in LIMA-AI-OS. Implement only the exact LIMA-side metadata-only built-in provider SDK client authority contract slice. Do not add provider SDK clients, SDK dependencies, vendor SDK imports, direct provider SDK implementation, LIMA-owned provider endpoint resolution execution, LIMA-owned network calls, LIMA-owned direct provider egress, secret lookup, credential-value access, provider token/API key access, provider configuration changes, fallback, consumer production runtime integration, connectors, physical-world behavior, product readiness, or final public API freeze claims.
