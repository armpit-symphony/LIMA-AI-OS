# V1-G37 Consumer Integration Patch-Preview Approval Request
...
This is an approval request only. It does not approve implementation, edit `lima/` runtime files, edit Sparkbot, edit Arc-Bot-shell, create consumer test files, persist raw patch bodies, apply patches, call adapter symbols, import consumer runtimes, wire shells, call providers/models, read secrets, execute tools, invoke connectors, execute browser/network/file/device/robotics/physical-world behavior, run scheduled tasks, send external messages, persist raw sensitive content in LIMA evidence, or claim product readiness.
...
Approval must be explicit before implementation begins. V1-G36, readiness rollups, decision matrices, broad product direction, or this request packet do not count as implementation approval.

## Proposed V1-G37 Objective

Implement the smallest LIMA-side consumer integration patch-preview evidence slice.

The slice should add deterministic LIMA-side docs/tests/fixtures that preview exact future consumer integration edit intent for Sparkbot and Arc-Bot-shell without mutating consumer repositories or persisting raw patch bodies. It may record sanitized file-intent metadata, future edit categories, risk checks, rollback checkpoints, and stop conditions. It must remain patch-preview metadata only and must not edit consumer repositories, wire shells, call adapter symbols, import consumer runtimes, call providers/models, access credentials, invoke connectors, use network/browser/device/robotics/physical-world behavior, or claim product readiness.
...
No `lima/` runtime files may be created, edited, removed, or renamed in V1-G37.

No Sparkbot or Arc-Bot-shell files may be created, edited, removed, or renamed in V1-G37.

Any other file requires a new gate update before implementation.

## Allowed Behavior If Approved

V1-G37 may add only deterministic LIMA-side docs/tests/fixtures that define sanitized patch-preview metadata.

Allowed if approved:

- add one LIMA-side consumer integration patch-preview fixture for Sparkbot and Arc-Bot-shell
- add one focused LIMA-side patch-preview test
- define future consumer edit intent categories without applying edits
- define future candidate consumer file refs without mutating those files
- record no raw patch body persistence confirmation
- link V1-G36 bounded design evidence, audit, authority-chain audit, readiness rollup, and next-lane matrix
...
- enforce proof-not-edit-authority, proof-not-integration-authority, and proof-not-product-readiness confirmations
...
- LIMA patch-preview fixture records `CANDIDATE_ONLY`
...
- Sparkbot patch-preview record exists and remains preview-only
- Arc-Bot-shell patch-preview record exists and remains preview-only
- V1-G36 bounded design evidence is linked
- no raw patch bodies are persisted
- future consumer repository edits, consumer integration, shell wiring, provider/model, connector/browser/network, physical-world, and product-readiness gates remain blocked
...
- proof-not-edit-authority and proof-not-integration-authority confirmations are enforced
...
Rollback must remove only the exact approved V1-G37 docs/tests/fixtures:
...
- file scope exceeds the approved V1-G37 files
...
- raw patch bodies are persisted
- patches are applied
...
- Consumer integration patch-preview evidence approved: no.
- Consumer integration patch-preview evidence added: no.
...
If approved, create branch `v1-g37-consumer-integration-patch-preview` in LIMA-AI-OS. Implement only the exact consumer integration patch-preview evidence slice. Do not edit runtime files, edit consumer repos, apply patches, wire shells, call providers/models, access secrets, invoke connector/browser/network behavior, add physical-world behavior, persist raw sensitive content in LIMA evidence, or claim product readiness.
