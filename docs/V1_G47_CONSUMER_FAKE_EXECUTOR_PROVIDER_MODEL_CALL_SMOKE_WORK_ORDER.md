# V1-G47 Consumer Fake-Executor Provider Model Call Smoke Work Order
...
Prepare an operator decision gate for consumer fake-executor smoke tests against the V1-G46 public harness wrapper.

The requested future implementation would add focused Sparkbot and Arc-Bot-shell tests/fixtures that import the V1-G46 public `lima.harness` execution wrapper and call it with fake in-process provider executors only. This work order does not approve or implement that slice.
...
- `tests/fixtures/runtime_extraction/v1_g47_consumer_fake_executor_provider_model_call_smoke_approval_request.json`
- `tests/test_v1_g47_consumer_fake_executor_provider_model_call_smoke_approval_request.py`
...
- `python -m pytest -q tests/test_v1_g47_consumer_fake_executor_provider_model_call_smoke_approval_request.py -p no:cacheprovider`
- `python -m pytest -q tests/test_v1_g47_consumer_fake_executor_provider_model_call_smoke_approval_request.py tests/test_v1_g46_live_provider_model_call_execution.py tests/test_v1_g22_final_public_api_freeze.py -p no:cacheprovider`
...
Ask the operator to approve, revise, or pause the V1-G47 implementation request. Implementation must not begin until the operator records `Approve-V1-G47` with the exact approval wording from the request.
